/**
 * Custom WebSocket STT Client
 * Connects to any compatible WebSocket transcription endpoint.
 *
 * Features:
...
export class CustomSTTClient {
    ...
    /**
     * Connect to the custom WebSocket endpoint
     */
    connect(config) {
        const { apiKey, wsUrl } = config;
        this.apiKey = apiKey;
        this._config = config;
        this._intentionalDisconnect = false;
        this._reconnectAttempts = 0;
        ...
        if (!wsUrl) {
            ...
            this.onError?.('WebSocket URL is required. Please add it in Settings.');
            return;
        }
    ...
    _doConnect(config, carryoverContext = null) {
        const { apiKey, wsUrl, sourceLanguage, targetLanguage, customContext, endpointDelayMs, speakerDiarization } = config;

        this._setStatus('connecting');
        console.log('[CustomSTT] Connecting to', wsUrl);

        let newWs;
        try {
            newWs = new WebSocket(wsUrl);
            console.log('[CustomSTT] WebSocket created, readyState:', newWs.readyState);
        } catch (err) {
            console.error('[CustomSTT] Failed to create WebSocket:', err);
            this._setStatus('error');
        ...
        newWs.onopen = () => {
            ...
            console.log('[CustomSTT] WebSocket OPEN' + (wasReconnect ? ' (reconnect)' : ''));
            ...
            console.log('[CustomSTT] Sending config (model:', configMsg.model, ')');
            newWs.send(JSON.stringify(configMsg));

            // Make-before-break: close old WS AFTER new one is ready
            const oldWs = this.ws;
            if (oldWs && oldWs !== newWs) {
                console.log('[CustomSTT] Seamless switch: closing old WebSocket');
                try {
            ...
            console.log('[CustomSTT] Connected and config sent');
        ...
        newWs.onmessage = (event) => {
            ...
            try {
                ...
                console.error('[CustomSTT] Failed to parse response:', err);
            }
        };

        newWs.onerror = (event) => {
            if (newWs._isOld) return;
            console.error('[CustomSTT] WebSocket ERROR:', event);
        ...
        newWs.onclose = (event) => {
            ...
            if (newWs._isOld) {
                console.log('[CustomSTT] Old WebSocket closed (expected)');
                return;
            }

            console.log('[CustomSTT] WebSocket CLOSED, code:', event.code, 'reason:', event.reason, 'wasClean:', event.wasClean);
            this.isConnected = false;
            ...
            if (event.code === 1000) {
                ...
                this._setStatus('error');
                this.onError?.('Access denied. Please check your account or subscription.');
            } else {
    ...
    finalize() {
        ...
        try {
            ...
            console.warn('[CustomSTT] finalize failed:', err);
        }
    }
    ...
    _seamlessReset() {
        ...
        console.log('[CustomSTT] Seamless session reset (every 3 min)');
    ...
    /**
     * Build the full context object from the rich context shape:
    ...
    _handleApiError(data) {
        ...
        console.error('[CustomSTT] API error:', code, message);
        ...
        if (code === 401) {
            ...
            userMessage = 'Insufficient credits. Check your account.';
        } else if (code === 400) {
    ...
    _tryReconnect(reason) {
        ...
        console.log(`[CustomSTT] Reconnecting (${this._reconnectAttempts}/${MAX_RECONNECT}) in ${delay}ms...`);
        this._setStatus('connecting');
