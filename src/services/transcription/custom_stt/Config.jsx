...
export function Config(props) {
    ...
    const [config, setConfig] = useConfig(
        instanceKey,
        {
            [INSTANCE_NAME_CONFIG_KEY]: t('services.transcription.custom_stt.title'),
            wsUrl: '',
            apiKey: '',
            endpointDelayMs: 250,
            batchIntervalMs: 100,
    ...
    return (
        config !== null && (
            <>
                ...
                <div className='config-item'>
                    <Input
                        label={t('services.transcription.custom_stt.ws_url')}
                        labelPlacement='outside'
                        placeholder='wss://your-provider.com/transcribe'
                        value={config.wsUrl ?? ''}
                        variant='bordered'
                        classNames={{ label: 'text-xs text-default-500 pb-1' }}
                        onValueChange={v => setConfig({ ...config, wsUrl: v })}
                    />
                </div>
                <div className='config-item'>
                    <p className='text-xs text-default-400'>
                        {t('services.transcription.custom_stt.ws_url_hint')}
                    </p>
                </div>
                <div className='config-item'>
                    <Input
                        label={t('services.transcription.custom_stt.api_key')}
                        labelPlacement='outside'
                ...
                <div className='config-item'>
                    <p className='text-xs text-default-400'>
                        {t('services.transcription.custom_stt.api_key_hint')}
                    </p>
                </div>

                {/* Endpoint delay */}
                <div className='config-item-row flex-col gap-1'>
                    <p className='text-sm'>{t('services.transcription.custom_stt.endpoint_delay')}</p>
                    ...
                    <p className='text-xs text-default-400'>{t('services.transcription.custom_stt.endpoint_delay_hint')}</p>
                </div>

                {/* Batch interval */}
                <div className='config-item-row flex-col gap-1'>
                    <p className='text-sm'>{t('services.transcription.custom_stt.batch_interval')}</p>
                    <div className='flex items-center gap-3'>
                        <input
                    ...
                    <p className='text-xs text-default-400'>{t('services.transcription.custom_stt.batch_interval_hint')}</p>
                </div>
                ...
                <div className='config-item'>
                    <div className='flex items-center justify-between w-full'>
                        <div className='flex flex-col gap-0.5'>
                            <p className='text-sm'>{t('services.transcription.custom_stt.speaker_diarization')}</p>
                            <p className='text-xs text-default-400'>{t('services.transcription.custom_stt.speaker_diarization_hint')}</p>
                        </div>
                        <Switch
                            size='sm'
